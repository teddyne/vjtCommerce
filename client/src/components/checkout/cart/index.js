...
const Cart = ({ currentUser }) => {
  ...
  const handleChangeQuantity = async (cart, type) => {
    ...
    setCarts(user.carts)
    setTotalPrice(totalPrice + (type === QualityActionEnum.MINUS ? -cart.price : cart.price))
    updateLocalStorage(user.carts, LocalStorageType.CART)
  }

  const updateQuantity = async (cart, type) => {
    try {
      const payload = {
        quantity: cart.quantity + (type === QualityActionEnum.MINUS ? -1 : 1)
      }
  ...
  return _.isEmpty(carts) ? <NoItem img={noCarts} imgTitle='Không có sản phẩm' text={'Không có sản phẩm nào trong giỏ hàng!'} /> :
    <Row className='cart'>
      <Col lg={9}>
        <div className='cart-list'>
          {_.map(carts, (cart) => {
            return <CartItem
                      key={cart._productId}
                      cart={cart}
                      onMinus={() => handleChangeQuantity(cart, QualityActionEnum.MINUS)}
                      onAdd={() => handleChangeQuantity(cart, QualityActionEnum.ADD)} />
          })}
        </div>
      </Col>
      <Col lg={3}>
